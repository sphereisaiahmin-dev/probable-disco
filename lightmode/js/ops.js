...
super(...arguments);
const op=this;
const attachments=op.attachments={"edgedetect_frag":"IN vec2 texCoord;\nUNI sampler2D tex;\nUNI float amount;\nUNI float width;\nUNI float strength;\nUNI float texWidth;\nUNI float texHeight;\nUNI float mulColor;\nconst vec4 lumcoeff=vec4(0.299,0.587,0.114,0.);\nvec3 desaturate(vec3 color)\n{\nreturn vec3(dot(vec3(0.2126,0.7152,0.0722),color));\n}\n{{CGL.BLENDMODES3}}\nvoid main()\n{\n\n\n\nfloat pixelX=(width+0.01)*4./texWidth;\nfloat pixelY=(width+0.01)*4./texHeight;\nvec2 tc=texCoord;\n\ntc.x+=1./texWidth*0.5;\ntc.y+=1./texHeight*0.5;\n\n\nfloat count=1.;\nvec4 base=texture(tex,texCoord);\n vec4 horizEdge=vec4(0.);\n horizEdge-=texture(tex,vec2(tc.x-pixelX,tc.y-pixelY))*1.;\n horizEdge-=texture(tex,vec2(tc.x-pixelX,tc.y))*2.;\n horizEdge-=texture(tex,vec2(tc.x-pixelX,tc.y+pixelY))*1.;\n horizEdge+=texture(tex,vec2(tc.x+pixelX,tc.y-pixelY))*1.;\n horizEdge+=texture(tex,vec2(tc.x+pixelX,tc.y))*2.;\n horizEdge+=texture(tex,vec2(tc.x+pixelX,tc.y+pixelY))*1.;\n vec4 vertEdge=vec4(0.);\n vertEdge-=texture(tex,vec2(tc.x-pixelX,tc.y-pixelY))*1.;\n vertEdge-=texture(tex,vec2(tc.x,tc.y-pixelY))*2.;\n vertEdge-=texture(tex,vec2(tc.x+pixelX,tc.y-pixelY))*1.;\n vertEdge+=texture(tex,vec2(tc.x-pixelX,tc.y+pixelY))*1.;\n vertEdge+=texture(tex,vec2(tc.x,tc.y+pixelY))*2.;\n vertEdge+=texture(tex,vec2(tc.x+pixelX,tc.y+pixelY))*1.;\n horizEdge*=base.a;\n vertEdge*=base.a;\n vec3 edge=sqrt((horizEdge.rgb/count*horizEdge.rgb/count)+(vertEdge.rgb/count*vertEdge.rgb/count));\nedge=desaturate(edge);\nedge*=strength;\nif(mulColor>0.)edge*=texture(tex,texCoord).rgb*mulColor*4.;\nedge=max(min(edge,1.),0.);\n\nvec4 col=vec4(edge,base.a);\noutColor=cgl_blendPixel(base,col,amount*base.a);\n}\n",};
const
    render = op.inTrigger("Render"),
    blendMode = CGL.TextureEffect.AddBlendSelect(op, "Blend Mode", "normal"),
    amount = op.inValueSlider("Amount", 1),
